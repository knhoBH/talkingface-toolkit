...
from talkingface.data.dataprocess.rh_dataprocess.keypoint2img import interpPoints, drawEdge
from scipy.spatial.transform import Rotation as R
from talkingface.data.dataprocess.rh_dataprocess.util.util import get_roi, get_roi_small_eyes, get_abso_mouth
from talkingface.data.dataprocess.rh_dataprocess.utils import face_utils
from talkingface.data.dataprocess.rh_dataprocess.util.util import openrate

import copy
import pdb


class FaceForeDataset(Dataset):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--worst_ref_prob', type=float, default=0.75,
                            help='probability to select worst reference image')
        ...
        parser.add_argument('--find_largest_mouth', action='store_true',
                            help='find reference image that open mouth in largest ratio')
    ...
    def __getitem__(self, index):
        if self.opt.dataset_name == 'face':
            video_path = self.data[index][
                1]  # os.path.join(self.root, 'pretrain', v_id[0] , v_id[1][:5] + '_crop.mp4'  )
            ...
            if self.opt.no_head_motion:
                lmark_path = os.path.join(self.root, self.video_bag, paths[0], paths[1],
                                          paths[2] + "_aligned_front.npy")
            else:
        ...
        # get reference
        ref_images, ref_lmarks, ref_coords = self.prepare_datas(real_video, lmarks, input_indexs, transform,
                                                                transform_L, scale)

        # get target
        tgt_images, tgt_lmarks, tgt_crop_coords = self.prepare_datas(real_video, lmarks, target_id, transform,
                                                                     transform_L, scale)
        ...
        for gg in target_id:
            ...
            tgt_templates_eyes.append(
                self.get_template(lmark, transform_T, self.output_shape, tgt_crop_coords, only_eyes=True))
            tgt_templates_mouth.append(
                self.get_template(lmark, transform_T, self.output_shape, tgt_crop_coords, only_mouth=True))
        ...
        if self.opt.warp_ani:
            ...
            ani_images, ani_lmarks, ani_coords = self.prepare_datas(ani_images, ani_lmarks, list(range(len(target_id))),
                                                                    transform, transform_L, scale)
            cropped_images, cropped_lmarks, _ = self.prepare_datas(cropped_images, cropped_lmarks,
                                                                   list(range(len(target_id))), \
                                                                   transform, transform_L, scale,
                                                                   crop_coords=tgt_crop_coords)
        ...
        if self.opt.crop_ref:
            ...
            for warp_id, warp_ref in enumerate(warping_refs):
                ...
                warp_ref_template = torch.Tensor(
            ...
            if self.opt.warp_ani:
                for ani_lmark_id, ani_lmark_temp in enumerate(ani_lmarks_back):
                    ani_template = torch.Tensor(
                        self.get_template(ani_lmark_temp, transform_T, self.output_shape, ani_coords))
        ...
        if self.opt.isTrain:
            tgt_templates = torch.cat(
                [torch.Tensor(tgt_template).unsqueeze(0).unsqueeze(0) for tgt_template in tgt_templates], axis=0)
            tgt_templates_eyes = torch.cat(
                [torch.Tensor(tgt_template).unsqueeze(0).unsqueeze(0) for tgt_template in tgt_templates_eyes], axis=0)
            tgt_templates_mouth = torch.cat(
                [torch.Tensor(tgt_template).unsqueeze(0).unsqueeze(0) for tgt_template in tgt_templates_mouth], axis=0)
        else:
            tgt_templates = torch.cat(
                [torch.Tensor(tgt_template).unsqueeze(0).unsqueeze(0) for tgt_template in tgt_templates], axis=0)
            tgt_templates_eyes = torch.cat(
        ...
        input_dic = {'v_id': target_img_path, 'tgt_label': tgt_lmarks, 'tgt_template': tgt_templates,
                     'ref_image': ref_images, 'ref_label': ref_lmarks, \
                     'tgt_image': tgt_images, 'target_id': target_id, 'warping_ref': warping_refs,
                     'warping_ref_lmark': warping_ref_lmarks, \
                     'ori_warping_refs': ori_warping_refs, 'path': video_path}
        if self.opt.warp_ani:
            input_dic.update({'ani_image': ani_images, 'ani_lmark': ani_lmarks, 'cropped_images': cropped_images,
                              'cropped_lmarks': cropped_lmarks})
        if self.opt.crop_ref:
    ...
    def get_image_index(self, n_frames_total, cur_seq_len, max_t_step=4):
        ...
        offset_max = max(1, cur_seq_len - (
                    n_frames_total - 1) * t_step)  # maximum possible frame index for the first frame
        ...
        if self.opt.isTrain:
            ...
            ref_range = list(range(max(0, start_idx - max_range), max(1, start_idx - min_range))) \
                        + list(
                range(min(start_idx + min_range, cur_seq_len - 1), min(start_idx + max_range, cur_seq_len)))
    ...
    def load_pickle(self, opt):
        if self.opt.dataset_name == 'face':
            ...
            self.data = [d for d in self.data if
                         ('1_1' not in d[0] and '1_2' not in d[0] and '1_3' not in d[0]) or '11_' in d[0]]
